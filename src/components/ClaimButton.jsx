...
function ClaimButton({ period, index, type, receiverForm }) {
  ...
  useEffect(
    function () {
      ...
      let dy = d.getDate();
      let y = d.getFullYear();
      getInvestmentStatus(type, index)
        .then((result) => {
          setStatus(result);
  ...
  const claim = async (type, period) => {
    ...
    claimInvestments(type, period, receiver)
      .then(function (data) {})
      .catch(console.error);
  };
  ...
  if (!status) {
    ...
    if (r) {
      return (
        <div className={`btn btn-sm ${claiming ? "btn-disabled" : "btn-success"}`} onClick={() => claim(type, index)}>
          {claiming ? "Claiming" : "Claim"}
        </div>
      );
    }
    if (!r) {
      return <div className="btn btn-sm btn-disabled">Pending </div>;
    }
  }
  if (status) {
    return <div className="btn btn-sm btn-primary">Claimed</div>;
  }
}
